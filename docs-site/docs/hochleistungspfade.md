...
(GDScript2All) ist entweder ein Werkzeug oder ein Ausblick –
...
| Über einen externen Dienst | ja | nein |
...
- **Externer Dienst:** Ein externer Python-Dienst verwendet WebSocket.
  Shared Memory ist dort nicht verfügbar, weil kein gemeinsamer RAM existiert.
...
4. Transportwahl nach Daten + Größe: Kontrolle → WebSocket, große
   lokale Daten → Shared Memory (bis dahin: DataRef/Datei), externe Dienste →
   WebSocket.
