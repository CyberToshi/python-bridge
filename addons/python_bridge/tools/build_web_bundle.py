...
def _script_declared_deps(project: Path, include: bool) -> list[dict]:
    """Collects ``__bridge_deps__ = [...]`` declarations from workspace
    scripts (same convention as the desktop provisioner). Returns a list of
    {"script", "deps"} entries for bridge_deps.json."""
    if not include:
        return []
    # Local import of the addon introspection module (no package install
    # needed; the addon python dir is added to sys.path in main()).
    from python_bridge import introspection as _intro  # type: ignore
    ...
    if scripts.is_dir():
        for py in sorted(scripts.rglob("*.py")):
            if "__pycache__" in py.parts:
                continue
            try:
                deps = _intro.deps_from_source(py.read_text(encoding="utf-8"))
            except (OSError, UnicodeDecodeError):
                continue
            if deps:
                entries.append({"script": py.relative_to(scripts).as_posix(),
                                "deps": list(deps)})
    return entries
...
def _make_workspace_tar(staging: Path, out_tar: Path) -> str:
    with tarfile.open(out_tar, "w") as tar:
        for item in sorted(staging.rglob("*")):
            if item.is_dir():
                # Store the directory entry WITHOUT recursing (recursive=False);
                # plain tar.add() would duplicate every child, skipping dirs
                # entirely would lose empty workspace folders (modules/, ...).
...
def build(project: Path, out: Path, packages: list[str], local_pyodide: Path | None,
    ...
    # 2b) Script-declared dependencies (``__bridge_deps__``) as a manifest
    # for the worker: it loads these through the Pyodide package repo too.
    # Written BOTH next to the export files and into the workspace tar -
    # the worker reads it from /workspace (MEMFS).
    addon_py = project / "addons" / "python_bridge" / "python"
    deps_entries: list[dict] = []
    if addon_py.is_dir():
        sys.path.insert(0, str(addon_py))
        try:
            deps_entries = _script_declared_deps(project, include=True)
        finally:
            sys.path.remove(str(addon_py))
    deps_json = json.dumps(deps_entries, indent=2, ensure_ascii=False)
    (staging / "bridge_deps.json").write_text(deps_json, encoding="utf-8")
    copied.append("bridge_deps.json")
    (out / "bridge_deps.json").write_text(deps_json, encoding="utf-8")
    if deps_entries:
        log.append("bridge_deps.json: %d script(s) declare packages" % len(deps_entries))

    # 2c) Build the tar AFTER the manifest landed in staging.
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--packages", default="numpy,scipy,pandas",
