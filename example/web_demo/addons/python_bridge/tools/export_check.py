...
def _web_bundle_state(project: Path, bundle_dir: Path) -> dict:
    ...
    if not bundle_dir.is_dir():
        for candidate in (project / "build" / "web_bridge", project / "build" / "web"):
...
def _check_common(project: Path, report: Report, fix: bool) -> None:
    ...
    # Godot 4.x trÃ¤gt Autoloads je nach Version per Pfad oder per UID ein
    # ("*uid://..."); beides ist gleichwertig. Entscheidend ist der aktivierte
    # Singleton-Eintrag ("*" = automatisch instanziieren).
    if re.search(r'^PythonBridge\s*=\s*"\*(?:res://[^"]+|uid://[^"]+)"',
                 project_godot, re.MULTILINE):
