...
`);
    // Ohne Bundle: das echte python_bridge-Paket direkt aus dem Addon ins
    // MEMFS schreiben (identisch zum Entpacken des workspace bundles).
    pyodide.runPython("__import__('os').makedirs('/workspace/bridge/python_bridge', exist_ok=True)");
    const bridgeSrc = fileURLToPath(
      new URL("../addons/python_bridge/python/python_bridge", import.meta.url));
    for (const name of readdirSync(bridgeSrc)) {
      if (!name.endsWith(".py")) continue;
      pyodide.FS.writeFile("/workspace/bridge/python_bridge/" + name,
        new Uint8Array(readFileSync(resolve(bridgeSrc, name))));
    }
    skip("virtuelles-fs: bundle", "kein --bundle angegeben; python_bridge-Paket direkt ins MEMFS kopiert");
  }
