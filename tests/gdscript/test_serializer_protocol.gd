...
func test_binary_chunk_roundtrip() -> void:
	...
	big[0] = 1
	big[4095] = 255
	var enc := PythonBridgeSerializer.encode(big, chunks)
...
func test_text_frame_roundtrip() -> void:
	...
	var frame := PythonProtocol.build_frame(msg)
	assert_true(frame.has("text"))
	var parsed := PythonProtocol.parse_frame(frame["text"])
...
func test_binary_frame_roundtrip() -> void:
	...
	var parsed := PythonProtocol.parse_frame(frame["binary"])
...
func test_response_envelope() -> void:
	var ok := PythonProtocol.response_envelope(
		PythonProtocol.MSG_TASK_RESULT, "t1", "ok", 42, {}, 12)
	assert_eq(ok["status"], "ok")
	assert_eq(ok["ms"], 12)
	var err := PythonProtocol.response_envelope(
